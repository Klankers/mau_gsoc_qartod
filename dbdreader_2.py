"""
A copy of dbreader.py from https://github.com/smerckel/dbdreader/blob/master/dbdreader/dbdreader.py

All credit goes to the original author(s).

Breakpoints have been inserted into DBD for testing purposes.

Works fine as a standalone, assuming dbreader is installed in the current working environment.
"""
...
class DbdError(Exception):
    ...
    MissingCacheFileData = namedtuple(
    ...
    *p : variable length list of str
    ...
    def _keyFilename(self, key: str) -> str:
        ...
        if match:
            ...
            n = sum(
                [int(i) * 10**j for i, j in zip(number_fields[1:], [8, 5, 3, 0])]
            )  # first field is '', so skip over
...
class DBD(object):
    ...
    def __init__(
        ...
        if dbdreader.decompress.is_compressed(filename):
            with dbdreader.decompress.CompressedFile(filename) as self.fp:
                self.headerInfo, parameterInfo, self.cacheFound, self.cacheID = (
                    self._read_header(self.cacheDir)
                )
        else:
        ...
        if not self.cacheFound:
            ...
            data = DbdError.MissingCacheFileData(
                {self.cacheID: [self.filename]}, self.cacheDir
            )
    ...
    def get_sync(
        ...
        *sync_parameters: variable length list of str
        ...
        if len(sync_parameters) == 2 and (
            isinstance(sync_parameters[1], list)
            or isinstance(sync_parameters[1], tuple)
        ):
    ...
    def _get(
        ...
        *parameters: str,
        ...
        if invalid_parameters and check_for_invalid_parameters:
            ...
            raise DbdError(
                value=DBD_ERROR_NO_VALID_PARAMETERS, mesg=mesg, data=invalid_parameters
            )
        ...
        if missing_parameters:
            logger.warning(
                f"Requested parameters not found: {','.join(missing_parameters)}."
            )
        ...
        if error_no:
            ...
            raise DbdError(
                value=DBD_ERROR_READ_ERROR,
                mesg=f"Decompression of {self.filename} failed with an '{s}' error.",
                data=error_no,
            )
        ...
        for i, p in enumerate(valid_parameters):
            ...
            if self._is_latlon_parameter(p):
                if (
                    discardBadLatLon and not return_nans
                ):  # discards and return nans is not compatible.
                    ...
                    timestamps[i], values[i] = numpy.compress(
                        condition, (timestamps[i], values[i]), axis=1
                    )
                if decimalLatLon:
    ...
    def _get_sync(
        ...
        *params: str,
        ...
        """
        timestamps, values = self._get(
            *params, decimalLatLon=decimalLatLon, discardBadLatLon=discardBadLatLon
        )
        t = timestamps[0]
        if t.shape[0] == 0:
    ...
    def _read_header(
        ...
        if result == DBD_ERROR_INVALID_DBD_FILE:
            ...
            raise DbdError(
                DBD_ERROR_INVALID_ENCODING, f"{self.filename} has an invalid encoding."
            )
        elif result == DBD_ERROR_DECOMPRESSION_ERROR:
            raise DbdError(
                DBD_ERROR_DECOMPRESSION_ERROR,
...
class MultiDBD(object):
    ...
    def __init__(
        ...
        self.parameterNames = dict(
            (k, self._getParameterList(v)) for k, v in self.dbds.items()
        )
    ...
    def get(
        ...
        *parameters: str,
        ...
        unavailable_parameters = self._get_valid_parameters(
            parameters, invert=True, global_scope=False
        )
    ...
    def get_sync(
        ...
        *parameters: variable length list of str
        ...
        if len(parameters) == 2 and (
            isinstance(parameters[1], list) or isinstance(parameters[1], tuple)
        ):
            # obsolete calling signature.
            logger.info("Calling signature of get_sync() has changed in version 0.4.0.")
    ...
    def _has_ctd_installed(self, ctd_type: str) -> bool:
        ...
        try:
            t, tctd = self.get(
                f"sci_{ctd_type}_timestamp", max_values_to_read=MAX_VALUES_TO_READ
            )
        except DbdError as e:
            logger.setLevel(loggerLevel)
            if (
                e.value == DBD_ERROR_NO_VALID_PARAMETERS
            ):  # If an error is raised, we expect this one
                result = False
    ...
    def has_parameter(self, parameter: str) -> bool:
        ...
        """
        return (
            parameter in self.parameterNames["sci"]
            or parameter in self.parameterNames["eng"]
        )
    ...
    def _safely_open_dbd_file(
        ...
        try:
            ...
            if e.value == DBD_ERROR_CACHEDIR_NOT_FOUND:
                ...
                _cacheID = list(e.data.missing_cache_files.keys())[
                    0
                ]  # we open a single file, so only one cache file can be missing.
    ...
    def _update_dbd_inventory(
        ...
        for fn in self.filenames:
            ...
            if result == "ignore" or result == "failed":
                ...
                dbd, result = self._safely_open_dbd_file(
                    ...
                    skip_initial_lines,
                    missing_cacheIDs,
                    check_for_compressed_cac=False,
                )
                if result == "ignore" or result == "failed":
                    filenames.remove(fn)
            if result != "ok":
        ...
        if missing_cacheIDs:
            ...
            for k, v in missing_cacheIDs.items():
                ...
                mesg += "\n"
            data = DbdError.MissingCacheFileData(
                dict([(k, v) for k, v in missing_cacheIDs.items()]), cacheDir
            )
            raise DbdError(DBD_ERROR_CACHE_NOT_FOUND, mesg=mesg, data=data)
            ...
            try:
                ...
                if e.value == DBD_ERROR_NO_DATA_TO_INTERPOLATE_TO:
                    ...
                    if continue_on_reading_error:
                        logger.warning(
                            f"Reading from {i.filename} returned an error ({e.data})."
                        )
                        continue
                    else:
                        raise e
